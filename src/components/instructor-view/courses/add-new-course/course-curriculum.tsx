...
import ReusableDialog from "@/components/Reusable-Components/Reusable-Dialog";
...
function CourseCurriculum() {
  ...
  //   // } = useContext(InstructorContext);
    ...
    function handleCourseTitleChange(event: ChangeEvent<HTMLInputElement>, currentIndex: number) {
    ...
    async function handleMediaBulkUpload(event:ChangeEvent<HTMLInputElement>) {
      ...
      try {
        ...
        if (response?.success) {
          ...
          cpyCourseCurriculumFormdata = [
            ...cpyCourseCurriculumFormdata,
            ...response?.data.map((item:unknown, index:number) => ({
              //@ts-ignore
              videoUrl: item?.url,
              //@ts-ignore
    ...
    const [files,setFiles] = useState<IFiles[]>([])
   	const handleAddDataToList = (data:IFiles[]) => {
		const mergeArray = [...data, ...files];
		setFiles(mergeArray);
	};
  ...
  return (
      <Card>
        <ReusableDialog
				  isOpen={open}
          closeDialog={() => setOpen(true)}
          triggerComponent={
          <ReusableButton onClick={() => setOpen(true)} btnText="upload" isLoading={false} />
        }
				dialogBody={
	        // @ts-ignore
					<FileUploader parentCallback={setFiles} handleCloseUploadFile={() => setOpen(false)} parentCallbackToFillData={handleAddDataToList}  /> 
				}
